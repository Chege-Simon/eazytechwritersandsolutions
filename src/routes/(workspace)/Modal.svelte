<script>
    ...
    export let button = 'Save Changes';
    export let button_color = 'primary';
  
    const modalClose = (option) => {
      open = false;
      if (onClosed) {
        onClosed(option);
      }
      ...
      <div class="modal-dialog" role="document" in:fly={{ y: -50, duration: 300 }} out:fly={{ y: -50, duration: 300, easing: quintOut }}>
        <div class="modal-content">
          ...
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-{button_color}" on:click={() => modalClose(button)}>{button}</button>
          </div>
        </div>
      </div>
    </div>
