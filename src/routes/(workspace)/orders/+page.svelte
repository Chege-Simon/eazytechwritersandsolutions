<script lang="js">
	...
	onMount(async () => {
		...
		unsubscribe = await pb.collection('orders').subscribe('*', async ({ action, record }) => {
			loading = true;
			if (action === 'create') {
				// Fetch associated client, task, education_level, service_type
				...
				const education_level = await pb.collection('education_levels').getOne(record.order.education_level);
...
<div class="card mt-5">
	<div class="card-body container">
		<div class="card-title row">
			<h4 class=" col-md-10">All Orders</h4>
			<a class="btn btn-primary col-md-2" href="/orders/new">
				<i class="mdi mdi-plus"></i>
				Place Order
			</a>
		</div>
		<p class="card-description">All Open orders</p>
					...
					<tr>
						...
						<td>
							{#if order.expand?.current_task?.status == 'open' || order.expand?.current_task == null}
								<i
									class="badge badge-warning">{order.expand?.current_task ? order.expand?.current_task.status : 'pending payment'}</i>
...
<!--								<a class="badge btn-inverse-success" style="cursor: pointer;" on:click={onShowAssignPopup}-->
<!--								>Assign</a-->
<!--								>-->
								<!-- svelte-ignore a11y-click-events-have-key-events -->
								<!-- svelte-ignore a11y-no-static-element-interactions -->
								<!-- svelte-ignore a11y-missing-attribute -->
								<a class="badge btn-inverse-primary" style="cursor: pointer;" href="/orders/{order.id}">Edit</a>
								<!-- svelte-ignore a11y-click-events-have-key-events -->
								<!-- svelte-ignore a11y-no-static-element-interactions -->
								<!-- svelte-ignore a11y-missing-attribute -->
								<a class="badge btn-inverse-danger" style="cursor: pointer;"
						...
						{:else if (order.expand?.current_task?.status == 'client_review' || order.expand?.current_task?.status == 'closed') && ($currentUser.role.includes('Client') || $currentUser.role.includes('Admin')) }
							<td>
								<a
									href={pb.files.getUrl(order.expand?.current_task, order.expand?.current_task?.order_preview_files, { thumb: '100x300f' })}
									target="_blank"
									rel="noreferrer"
						...
						{:else if ($currentUser.role.includes('Client') || $currentUser.role.includes('Admin')) && order.amount_paid >= order.amount}
							<td>
								<a
									href={pb.files.getUrl(order.expand?.current_task, order.expand?.current_task?.order_preview_files) + '?download=1'}
									target="_blank"
									rel="noreferrer"
...
<Modal open={showDeletePopup} onClosed={(option) => onPopupClose(option)} title={"Delete Order"} button="Delete"
			 button_color="danger">
